...
See `docs/gates/G7A.md` for the deferred re-entry condition.
...
| `PHASE-7A` / `G7A` | Conditional PASS for the approved synthetic/test-only point-kinetics/quasi-static amplitude, I-135/Xe-135 history, stable timestep integration, controlled spatial coupling, and history scenarios. | T3, long histories, timestep convergence, and nonnegative inventories passed; an approved Phase 7 reference/golden package and G7A re-entry remain required for an unconditional disposition. |
