...
| `XSEC-03-R2` | `BLOCKED` — [`XSEC-03-R2.md`](tasks/XSEC-03-R2.md) and its [path-free run/semantic manifest](../reference/manifests/xsec-03-r2-run-semantic-v1.json) prove four clean SPH-preserving DRAGON reruns: source assertions, normal completion, source convergence fingerprints, per-state capture byte identity, state distinction, and selector prechecks passed. Semantic decode stopped before values were read because the selected MACROLIB contains multiple mixture elements while mapping v1/v2 never names the required one homogenized whole-cell mixture or its locator. `XSEC-MIX-01` subsequently proved that the approved TCWUX11 EDI boundary has ten mixtures and no compatible source-named whole-cell output. No field, pack, golden/reference, runtime, full-core, or Unity admission occurred. | Historical blocked result preserved. Do not rerun under mapping-v2/deck-v3: a separately owner-approved source/deck and mapping task must first explicitly create and name one whole-cell homogenized output, prove its semantics and source regression, then authorize a fresh XSEC-03-R2 rerun. |
| `XSEC-MIX-01` | `BLOCKED` — [`XSEC-MIX-01.md`](tasks/XSEC-MIX-01.md), its [owner approval](tasks/XSEC-MIX-01-OWNER-APPROVAL.md), [blocked selection decision](spec/xsec-source-mixture-selection-v1.md), and [path-free manifest](../reference/manifests/xsec-mix-01-selection-v1.json) resolve the missing selection authority without inventing a mixture. The official TCWUX11 EDI block maps 31 regions to ten mixture IDs, and both selected MACROLIBs expose ten mixtures/volumes with no source-named whole-cell record. No mixture, field, pack, golden/reference, runtime, full-core, or Unity admission occurred. | None. A separately owner-approved source/deck and mapping task must explicitly create and name a single whole-cell homogenized output, prove its semantics and source regression, and then schedule a fresh XSEC-03-R2 rerun. |
