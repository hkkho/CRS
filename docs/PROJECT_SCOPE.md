...
| `XSEC-FIELD-01` | `AUTHORIZED TASK DEFINITION` — [owner approval](tasks/XSEC-FIELD-01-OWNER-APPROVAL.md) and [request](tasks/XSEC-FIELD-01-REQUEST.md) define the bounded required-field source export resolution; its independent high task-definition review is PASS, telemetry `UNVERIFIED`. It may create only a reviewed deck v5/mapping v4 route that preserves the original EDI/SPH/assertion path and proves `EFIS`/`DIFF` labels with official meaning and units, or a precise fail-closed blocker. No numerical value, pack, runtime, golden/reference, full-core, or Unity admission is authorized. | Execute XSEC-FIELD-01 as the next task. It must not substitute `H-FACTOR`, alter XSEC-03-R3 historical evidence, or decode numerical arrays. |
