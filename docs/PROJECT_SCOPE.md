...
| `XSEC-02` / `XSEC-02-R1` | `COMPLETE` — the historical [`XSEC-02.md`](tasks/XSEC-02.md) blocked report preserves its original artifact-layout finding; [`XSEC-02-R1.md`](tasks/XSEC-02-R1.md) records the recovered fresh wrapper T3 after `TEST-INFRA-02`: Core `208/208`, Golden `26/26`, and CLI `38/38`, all with zero failures/skips. The reviewed [candidate mapping specification](spec/xsec-source-runtime-mapping-v1.md) and [path-free machine record](../reference/manifests/xsec-02-mapping-spec-v1.json) bind a TCWU11/JEFF-3.1 candidate only: source procedure hashes, the two-group 4 eV order, source fields/units, post-EDI/pre-SPH export, bounded deck deltas, and Core/full-core exclusions. Same-reviewer high review is `PASS` with telemetry `UNVERIFIED`; no DRAGON rerun, runtime pack, DONJON core, golden case, release, or Unity use is admitted. | XSEC-03 attempted the required offline reproduction and is blocked on a frozen deck conflict. `XSEC-DECK-01` is next; `XSEC-CORE-01` remains required before XSEC-04. |
| `XSEC-03` | `BLOCKED` — [`XSEC-03.md`](tasks/XSEC-03.md) records no valid DRAGON result or semantic export. The frozen TCWU11 procedure asserts WLUP-specific multiplication lexemes (`1.121035`, `0.9414081`, `1.121052`, `1.075327`), while the exact JEFF procedure asserts `1.118478`, `0.9420414`, `1.118481`, `1.073615`; shared `assertS` aborts at relative difference `>= 1.0E-4`. Assertion deletion, replacement, or TCWUX11 adoption is not among the approved XSEC-02 deck deltas. Raw failed inputs/logs stay external. | `XSEC-DECK-01` is the next eligible bounded authority task: select and review one exact executable JEFF deck basis. XSEC-03 cannot resume until that decision completes; XSEC-CORE-01 and XSEC-04 remain ineligible. |
