...
| `XSEC-DECK-01` | `COMPLETE` — [`XSEC-DECK-01.md`](tasks/XSEC-DECK-01.md), its [owner decision](tasks/XSEC-DECK-01-OWNER-APPROVAL.md), [v2 deck authority](spec/xsec-jeff-lattice-deck-v2.md), and [path-free source record](../reference/manifests/xsec-deck-01-source-admission-v2.json) select official Version5 JEFF TCWUX11, retain its four source assertions, preserve v1 field/unit/group mapping, and permit only two SPH omissions plus read-only decoding of source `res`. T3 passed Core `208/208`, Golden `26/26`, CLI `38/38`; independent high review is `PASS`, telemetry `UNVERIFIED`. It creates no numerical result, runtime pack, golden/reference, full-core, release, or Unity admission. | Define and execute `XSEC-03-R1`: fresh v2 DRAGON semantic-export T6 reproduction. |
