...
| `XSEC-02` | `BLOCKED` — [`XSEC-02.md`](tasks/XSEC-02.md), the [candidate mapping specification](spec/xsec-source-runtime-mapping-v1.md), and its [path-free machine record](../reference/manifests/xsec-02-mapping-spec-v1.json) bind a TCWU11/JEFF-3.1 candidate only: source procedure hashes, the two-group 4 eV order, source fields/units, post-EDI/pre-SPH export, bounded deck deltas, and Core/full-core exclusions. Same-reviewer high review is `PASS` with telemetry `UNVERIFIED`; no DRAGON rerun, runtime pack, DONJON core, golden case, release, or Unity use is admitted. Its original mandatory artifact-wrapper T3 failure was recovered by separately completed `TEST-INFRA-02`: the fresh wrapper now passes Core `208/208`, Golden `26/26`, and CLI `38/38`, all with zero failures/skips. | Define and execute `XSEC-02-R1` as a revalidation-only task to record the recovered required T3 and final XSEC-02 disposition. `XSEC-03` is not eligible until that checkpoint passes. `XSEC-CORE-01` is registered in the chain as a separate topology/static-core authority and must complete before XSEC-04. |
