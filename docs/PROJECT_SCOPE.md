...
| `XSEC-02` | `BLOCKED` — [`XSEC-02.md`](tasks/XSEC-02.md), the [candidate mapping specification](spec/xsec-source-runtime-mapping-v1.md), and its [path-free machine record](../reference/manifests/xsec-02-mapping-spec-v1.json) bind a TCWU11/JEFF-3.1 candidate only: source procedure hashes, the two-group 4 eV order, source fields/units, post-EDI/pre-SPH export, bounded deck deltas, and Core/full-core exclusions. Same-reviewer high review is `PASS` with telemetry `UNVERIFIED`; no DRAGON rerun, runtime pack, DONJON core, golden case, release, or Unity use is admitted. Mandatory artifact-wrapper T3 is failing only in CLI: Core `208/208` and Golden `26/26` pass, while CLI fails `36/38` because Phase 8 approved parameter packs are absent from the temporary artifact layout; direct T3 passes Core `208/208`, Golden `26/26`, CLI `38/38`. | Define and execute `TEST-INFRA-02` as a separate non-physics artifact-portability recovery, then rerun XSEC-02 T3. `XSEC-03` is not eligible until that T3 passes. `XSEC-CORE-01` is registered in the chain as a separate topology/static-core authority and must complete before XSEC-04. |
