...
| `PHASE-7A` / `G7A` | Conditional PASS for the approved synthetic/test-only point-kinetics/quasi-static amplitude, I-135/Xe-135 history, stable timestep integration, controlled spatial coupling, history scenarios, and the bounded `P7-T06` synthetic authority consumer. | T3, long histories, timestep convergence, nonnegative inventories, exact independent regeneration, manifest binding, and Core consumption passed; no external or production baseline was admitted. Phase 7B remains optional/deferred. |
