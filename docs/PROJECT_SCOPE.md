...
implemented and T3-validated. G7A is now a conditional pass for the
synthetic/test-only Core boundary after long-history, timestep-convergence, and
nonnegative-inventory evidence; no applicable Phase 7 reference/golden
authority exists, so no production or external-reference claim is promoted.
`P7-T06` is activated as the bounded reference/golden authority research and
manufactured-fallback task; see its routing record and
`docs/gates/G7A.md` for the deferred re-entry condition.
...
| `G7A` | `CONDITIONAL PASS` | `docs/gates/G7A.md` records focused `2/2` long-history/refinement tests, full T3 CLI `5/5`, Core `189/189`, Golden `19/19`, format/diff checks, and the golden audit (`5` existing artifacts, `0` applicable Phase 7 files). Same-context independent high review final `CONDITIONAL PASS` with no actionable code findings and telemetry `UNVERIFIED`; evidence is synthetic/test-only and does not promote a reference or golden baseline. | `P7-T06` is now the bounded owner-authorized reference/golden admission task; G7A re-entry remains required for an unconditional comparison disposition. Phase 7B remains optional/deferred until approved temperature/purity feedback data enables a bounded task. |
| `P7-T06` | `ACTIVATED TASK-DEFINITION / ROUTING` | [`docs/tasks/P7-T06-REFERENCE-GOLDEN-AUTHORITY-CHAIN.md`](tasks/P7-T06-REFERENCE-GOLDEN-AUTHORITY-CHAIN.md) defines a deep-research-first external-source route and a clearly labeled independent manufactured/synthetic fallback. It changes no runtime, equations, units, tolerances, public schemas, or golden artifacts at activation time. Applicable literature rows are `S1-R09`, `S4-R05`, `S4-R06`, and `S5-R08`; they remain context/candidate evidence until exact reproduction and admission. | Execute `P7-T06` and write `docs/tasks/P7-T06.md`; then perform G7A re-entry for the admitted or synthetic-only scope. |
...
| `PHASE-7A` / `G7A` | Conditional PASS for the approved synthetic/test-only point-kinetics/quasi-static amplitude, I-135/Xe-135 history, stable timestep integration, controlled spatial coupling, and history scenarios. `P7-T06` is activated to address the missing reference/golden evidence. | T3, long histories, timestep convergence, and nonnegative inventories passed; `P7-T06` must produce an admitted reproducible case or explicitly synthetic manufactured package before G7A re-entry can seek an unconditional disposition. |
