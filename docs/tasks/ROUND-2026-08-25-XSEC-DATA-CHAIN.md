...
| `XSEC-03` | Reproduce at least one admitted DRAGON5 CANDU lattice/depletion case and export the approved group constants for fresh, equilibrium-like, refuelled/perturbed, and control/poison states where the specification supports them. | `XSEC-02` and completed `XSEC-DECK-01`; exact tools/data/decks/lexemes/hashes/run logs/convergence; T6 repeat. Unsupported states are reported, not fabricated. |
