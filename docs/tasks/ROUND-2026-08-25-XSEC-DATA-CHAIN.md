...
| `XSEC-SPH-02` | Prove a source-supported post-EDI/pre-SPH capture that retains the original SPH state transition and all four TCWUX11 assertions. | Completed `XSEC-RUNNER-01`; two clean candidate and baseline runs; exact object/timing/serialization identities; independent high review. No source-data export or runtime admission. |
